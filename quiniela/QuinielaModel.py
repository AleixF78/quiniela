...
class QuinielaModel:
    ...
    def train(self, train_data):  
        ...
        df_current.to_csv('models/df_current.csv', index=False)
        df_previous.to_csv('models/df_previous.csv', index=False)
        pass

    def predict(self, predict_data):
        df_1 = process_and_filter_data(predict_data)
        X = df_1.iloc[:, :-1] 
        df_current = pd.read_csv('models/df_current.csv')
        df_previous = pd.read_csv('models/df_previous.csv')
        parameters_to_calculate = ['Pts', 'GD']
        X_process = process_data(X,df_previous,df_current,parameters_to_calculate)
